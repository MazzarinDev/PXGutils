...
import { users, guilds, guildMembers, tasks, taskCategories, alarms, notifications, tags, taskTags, taskHistory } from "../drizzle/schema";
import { eq, and, desc, asc } from "drizzle-orm";

/**
 * Simple auth router - Login by character name only
 */
  ...
  listByUser: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      return await db
        .select()
        .from(guilds)
        .where(eq(guilds.id, guildIds[0])); // Simplified for now
    }),
  ...
  listActive: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      return await db
        ...
        .orderBy(asc(alarms.triggerTime));
    }),
});
